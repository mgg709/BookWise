  ...
  <div class="book-view-content">
    <div class="book-details">
      ...
      <div class="book-reviews">
          <span>Reviews</span>
          <NormalButton textButton="Add review" @click="redirectToWriteReview(book.title)"></NormalButton>
...
methods: {
  ...
  async getBookByCategory(){
    const route = useRoute();
    const {data} = await axios.get(`http://localhost:8080/search/category/${route.params.title}`);
    this.books = data;
  ...
  redirectToWriteReview(titlebook) {
      this.$router.push(`/write/${titlebook}`);
    }
},
created(){
  this.getBookByTitle();
